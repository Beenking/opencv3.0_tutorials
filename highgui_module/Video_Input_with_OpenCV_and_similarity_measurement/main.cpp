...
int main(int argc, char *argv[], char* window_name){
	...
	if(!captUndTst.isOpened()){
		cout << "Could not open cast test" << sourceCompareWith << endl;
		return -1;
	}

	Size refS = Size((int)captRefrnc.get(cv::CAP_PROP_FRAME_WIDTH),
		(int)captRefrnc.get(cv::CAP_PROP_FRAME_HEIGHT));
	Size uTSi = Size((int)captUndTst.get(cv::CAP_PROP_FRAME_WIDTH),
		(int)captUndTst.get(cv::CAP_PROP_FRAME_HEIGHT));

	if(refS!=uTSi){
		cout << "Input have differnt size .Closing." << endl;
		return -1;
	}
